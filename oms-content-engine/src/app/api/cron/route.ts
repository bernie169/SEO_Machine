...
export async function GET(req: NextRequest) {
  ...
  try {
    const message = await anthropic.messages.create({
      ...
      max_tokens: 3000,
Add daily cron route - scheduled article generation at 9am SAST      messages: [{ role: 'user', content: 'Write a news article about: "' + keyword + '"\n\nWeb research:\n' + searchContext + '\n\nRespond ONLY with JSON.' }],
    })
    ...
    const { error } = await supabase.from('news').insert({
      ...
      image: imageUrl, created_at: new Date().toISOString(),
    })

    if (error) throw new Error(error.message)

    return NextResponse.json({ success: true, keyword, title: parsed.title, slug, imageGenerated: !!imageUrl })
  } catch (e) {
